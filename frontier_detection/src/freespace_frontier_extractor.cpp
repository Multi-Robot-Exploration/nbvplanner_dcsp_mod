                                                   ...
                                                   frontier_detection::frontier_srv::Response &res)
{
    ...
    octomap::OcTree* oct;
    if (ros::service::call("firefly/nbvPlanner/get_map", Octosrv))
    {
        ROS_INFO("map accquired.");
        octomap::AbstractOcTree *tree = octomap_msgs::fullMsgToMap(Octosrv.response.map);
        oct = dynamic_cast<octomap::OcTree*>(tree);
    }

    this->SetOcTree(&oct);
    std::vector<octomap_frontiers3d::FreeSpaceFrontierRepresentative> frontierVector;
    this->Extract(frontierVector);

    // octomap_frontiers3d::FreeSpaceFrontierRepresentative frontier = frontierVector.back();
    // geometry_msgs::Pose pose;
    // pose.position = frontier.p;
    // res.frontier = pose; 
    ...
    res.frontiers = frontiers_; 
    res.total = frontierVector.size();

    visualize_Frontiers(frontiers_);
    /////////////////////////////////////////
...
void FreeSpaceFrontierExtractor::visualize_Frontiers(std::vector<geometry_msgs::Point> frontiers)
{
        ...
        for (int i = 0; i < frontiers.size(); i++)
        {
            ...
            marker.pose.position.x = frontiers.at(i).x;
            marker.pose.position.y = frontiers.at(i).y;
            marker.pose.position.z = frontiers.at(i).z;
